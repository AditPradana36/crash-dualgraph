...
EDGE_STYLE = {
    "sees": dict(linestyle="-", alpha=0.5, linewidth=1.0, color="white"),
    "mounted_with": dict(linestyle="--", alpha=0.7, linewidth=1.3, color="yellow"),
    "near": dict(linestyle=":", alpha=0.28, linewidth=0.8, color="cyan"),
}
...
def render_svg_overlay(image, seg_map, segments_info, objects, data, w, h, seg_alpha=0.55):
    ...
    for node_type in OBJECT_NODE_TYPES:
        for o in objects[node_type]:
            ...
            ax.scatter([o["cx_px"]], [o["cy_px"]], marker="o", s=70,
