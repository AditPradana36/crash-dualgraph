...
#
# One HeteroConv stack over SVG's node/edge types + TVG's node/edge types
# + the new same_location edge (see unified_graph.py for the merge and
# the svg_building/tvg_building rename that avoids the "building" name
# collision). Reuses SVGEncoder/TVGEncoder's own input-assembly logic
# rather than duplicating it — UnifiedEncoder owns its OWN embedders/
# projections (never shares weights with a standalone SVGEncoder/
# TVGEncoder instance) but the per-node-type feature assembly is
# identical to what those encoders already do, just writing into
# "svg_building"/"tvg_building" keys instead of "building" for the two
# renamed types.
# ─────────────────────────────────────────────────────────────────────────

from unified_graph import UNIFIED_SVG_NODE_TYPES, UNIFIED_TVG_NODE_TYPES, SAME_LOCATION_EDGE_TYPES

UNIFIED_NODE_TYPES = UNIFIED_SVG_NODE_TYPES + UNIFIED_TVG_NODE_TYPES

# SVG's edge types, renamed: only "building" (src or dst) becomes
# "svg_building" — every other SVG node type name is untouched.
def _rename_svg_edge(key):
    src, rel, dst = key
    ren = lambda nt: "svg_building" if nt == "building" else nt
    return (ren(src), rel, ren(dst))


def _rename_tvg_edge(key):
    src, rel, dst = key
    ren = lambda nt: "tvg_building" if nt == "building" else nt
    return (ren(src), rel, ren(dst))


UNIFIED_SVG_EDGE_TYPES = [_rename_svg_edge(k) for k in SVG_EDGE_TYPES]
# crash_history (ablation-only) excluded by default, same as TVGEncoder's
# own non-ablation path — added back conditionally in __init__ below.
UNIFIED_TVG_EDGE_TYPES_BASE = [_rename_tvg_edge(k) for k in TVG_EDGE_TYPES if k[1] != "crash_history"]
UNIFIED_TVG_EDGE_TYPES_ABLATION = [_rename_tvg_edge(k) for k in TVG_EDGE_TYPES]


class UnifiedEncoder(nn.Module):
    """Single HeteroConv stack over the merged SVG+TVG graph. Anchor for
    pooling is "incident" (TVG's per-point node) — arbitrary choice
    between it and "ego" since same_location now connects them every
    layer anyway, but a choice has to be made for _pool_and_anchor's
    single-anchor contract, and "incident" matches every other TVG-
    containing scenario's (B/C/D/E) convention of using the crash-point
    node as the anchor rather than the street-view viewpoint."""
    ...
    def __init__(self, hidden_dim=64, heads=4, num_layers=2, dropout=0.35,
        ...
        # TVG-side embedders/projections — same shapes as TVGEncoder, own weights.
        self.building_type_embed = CategoricalEmbedder(building_type_vocab, building_type_embed_dim)
        self.highway_embed = CategoricalEmbedder(highway_vocab, highway_embed_dim)
        self.height_module = MissingAwareContinuous(continuous_missing_dim)
        self.levels_module = MissingAwareContinuous(continuous_missing_dim)
        ...
        self.input_proj = nn.ModuleDict({
            "ego": nn.Linear(5, hidden_dim),
            "signage": nn.Linear(2 + 1 + cat_embed_dim, hidden_dim),
            "light_pole": nn.Linear(2 + 1 + cat_embed_dim, hidden_dim),
            "road_marking": nn.Linear(2 + 1 + cat_embed_dim, hidden_dim),
            "svg_building": nn.Linear(2 + 1, hidden_dim),
            "vegetation": nn.Linear(2 + 1, hidden_dim),
            "incident": nn.Linear(4 + highway_embed_dim, hidden_dim),
            "tvg_building": nn.Linear(6 + building_type_embed_dim + continuous_missing_dim * 2, hidden_dim),
            "intersection": nn.Linear(2, hidden_dim),
            "peer_incident": nn.Linear(1, hidden_dim),
        })

        tvg_edge_types = UNIFIED_TVG_EDGE_TYPES_ABLATION if use_ablation_edges else UNIFIED_TVG_EDGE_TYPES_BASE
        edge_types = UNIFIED_SVG_EDGE_TYPES + tvg_edge_types + SAME_LOCATION_EDGE_TYPES

        edge_dim_for = {}
        for key in UNIFIED_SVG_EDGE_TYPES:
            edge_dim_for[key] = SVG_EDGE_DIM[key[1]]
        for key in tvg_edge_types:
            rel = key[1]
            if rel in ("connects", "on_segment"):
                edge_dim_for[key] = (TVG_RAW_EDGE_DIM[rel] - 1) + highway_embed_dim
            else:
        ...
        self.convs = nn.ModuleList()
        self.norms = nn.ModuleList()
        node_types = UNIFIED_SVG_NODE_TYPES + [nt for nt in UNIFIED_TVG_NODE_TYPES
                                                if use_ablation_edges or nt != "peer_incident"]
        for _ in range(num_layers):
            conv_dict = {}
    ...
    def assemble_inputs(self, data):
        ...
        for nt, embedder in [("signage", self.signage_embed), ("light_pole", self.light_pole_embed),
            ...
            if data[nt].class_idx.shape[0] > 0:
                ...
                x_dict[nt] = torch.zeros((0, pos_area.shape[1] + embedder.embed.embedding_dim), device=pos_area.device)
        for nt in ["svg_building", "vegetation"]:
            x_dict[nt] = torch.cat([data[nt].x, data[nt].area_norm], dim=1)

        hw_idx = data["incident"].highway_type_idx
        hw_emb = self.highway_embed(hw_idx)
        x_dict["incident"] = torch.cat([data["incident"].x, hw_emb], dim=1)

        n_b = data["tvg_building"].x.shape[0]
        if n_b > 0:
            type_emb = self.building_type_embed(data["tvg_building"].type_idx)
            height_feat = self.height_module(data["tvg_building"].height, data["tvg_building"].height_missing)
            levels_feat = self.levels_module(data["tvg_building"].levels, data["tvg_building"].levels_missing)
            x_dict["tvg_building"] = torch.cat([data["tvg_building"].x, type_emb, height_feat, levels_feat], dim=1)
        else:
            dim = 6 + self.building_type_embed.embed.embedding_dim + self.height_module.proj.out_features * 2
            x_dict["tvg_building"] = torch.zeros((0, dim), device=x_dict["incident"].device)

        x_dict["intersection"] = data["intersection"].x
        if self.use_ablation_edges:
            x_dict["peer_incident"] = data["peer_incident"].x
    ...
    def assemble_edge_attrs(self, data):
        ...
        for key in self._edge_types:
            ...
            raw = data[key].edge_attr
            if rel in ("connects", "on_segment"):
                cat_col = 0 if rel == "connects" else 1
                edge_attr_dict[key] = embed_edge_attr_with_categorical(raw, cat_col, self.highway_embed)
            else:
                edge_attr_dict[key] = raw
        return edge_attr_dict
...
def build_model(scenario, fusion_dim=64, head_depth="linear", head_hidden=32, head_dropout=0.35,
    ...
    if scenario == "F":
        ...
        # both — svg_kwargs is allowed to win on those since callers so
        # far always pass identical values for both; only pass distinct
        # per-branch keys otherwise). use_ablation already folded into
